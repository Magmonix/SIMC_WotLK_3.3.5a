                              ...
                              const std::string& url )
{
  ...
  else if ( url.find( "evowow" ) != url.npos )
  {
    if ( ( cut_pt = url.find_first_of( "#=" ) ) != url.npos )
    {
...
void player_t::init_meta_gem( gear_stats_t& item_stats )
{
  ...
  else if (meta_gem == META_THUNDERING_SKYFLARE)
  {
    unique_gear_t::register_stat_proc(PROC_SPELL, RESULT_HIT_MASK, "thundering_skyflare", this, STAT_HASTE_RATING, 1, 480, 1, 6.0, 60.0);
  }
}
