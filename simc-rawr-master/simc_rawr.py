#!/usr/bin/python
import tkinter, tkinter.filedialog, shutil, os, subprocess, sys, traceback
import rawr2simc
...
def parse_config():
  ...
  try:
    config["config_file"] = "E:\WotLK\WoW - WotLK (Modern Textures)\Simc\simc-rawr-master\simc_rawr.conf"
  except IndexError:
...
def get_rawr_files():
  tk_root = tkinter.Tk()
  tk_root.withdraw()
  filenames = tkinter.filedialog.askopenfilenames(title="Choose Rawr character files", initialdir=config["rawr_char_dir"])
...
def do_simc():
  ...
  print("")
  print("Finished. Press any key to exit..." + os.linesep)
  exit_with_cleanup(0)
...
if __name__ == "__main__":
  try:
    ...
    print("SimC Rawr launcher encountered an error")
